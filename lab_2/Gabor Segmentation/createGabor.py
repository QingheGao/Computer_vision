...
def generateRotationMatrix(theta):
    ...
    rotMat = np.array([[np.cos(theta), -np.sin(theta)],
                       [np.sin(theta), np.cos(theta)]])# \\TODO: code the rotation matrix given theta.
    return rotMat
...
def createSin(rot_x, lamda, psi):
    ...
    sinCarrier = np.cos(2*np.pi*rot_x/lamda + psi )# # \\TODO: Implement the sine given rot_x, lamda and psi.
...
# sigmalist = [1,5,10]
# for index, k in enumerate(sigmalist):
#     plt.subplot(1,len(sigmalist),index+1)
#     s = createGabor(sigma =  k, theta = np.pi/2, lamda = 2.82, psi = 0, gamma=1 )[:,:,0]
#     plt.imshow(s,cmap='gray')
#     plt.title('$\sigma$ ='+str(k))
#
#     plt.xticks([])
#     plt.yticks([])
# plt.savefig('./figure/sigma.png',dpi=400,bbox_inches='tight')
# plt.show()

# thetalist = [0,np.pi/4,np.pi/2]
# titlelist = ['0','$\pi$/4','$\pi$/2']
# for index, k in enumerate(thetalist):
#     plt.subplot(1,len(thetalist),index+1)
#     s = createGabor(sigma =  5, theta = k, lamda = 2.82, psi = 0, gamma=1 )[:,:,0]
#     plt.imshow(s,cmap='gray')
#     plt.title('$\Theta$ ='+titlelist[index])
#     plt.xticks([])
#     plt.yticks([])
# plt.savefig('./figure/theta.png',dpi=400,bbox_inches='tight')
# plt.show()


# gammalist = [0.5,1,2]
# for index, k in enumerate(gammalist):
#     plt.subplot(1,len(gammalist),index+1)
#     s = createGabor(sigma =  5, theta = np.pi/2, lamda = 2.82, psi = 0, gamma=k )[:,:,0]
#     plt.imshow(s,cmap='gray')
#     plt.title('$\gamma$ ='+str(k))
#     plt.xticks([])
#     plt.yticks([])
# # plt.savefig('./figure/gamma.png',dpi=400,bbox_inches='tight')
# plt.show()