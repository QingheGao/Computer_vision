...
# lambdas = [ 2.82842712  5.65685425 11.3137085  22.627417   45.254834   90.50966799]
lambdas = [90.50966799]
...
for gaborFilter in gaborFilterBank:
    ...
    real_out =  cv2.filter2D(img_gray,-1,kernel=gaborFilter["filterPairs"][:,:,0]) # \\TODO: filter the grayscale input with real part of the Gabor
    imag_out =  cv2.filter2D(img_gray,-1,kernel=gaborFilter["filterPairs"][:,:,1])# \\TODO: filter the grayscale input with imaginary part of the Gabor
...
if smoothingFlag:
    ...
    for i in range(len(featureMags)):
        features[:,:,i] = (cv2.filter2D(featureMags[i].astype(np.float32),-1,3))
    #END_FOR
else:
...
plt.imshow(pixLabels,cmap='gray')
plt.axis("off")
plt.show()
